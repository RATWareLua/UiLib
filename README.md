...
Файлы лежат в `NierUI/<имя окна>/<профиль>.json`. Нет `writefile` у
исполнителя — сохранение просто не работает, меню при этом не ломается.
...
встроенных. **Кириллицы у неё нет**: подписи латиницей либо смена шрифта.
